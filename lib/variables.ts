...
export const cpu = Variable<string>("").poll(5000, "top -b -n 1", (out) => {
  const cpuUsage = out
    .split("\n")
...
export const ramGB = Variable<string>("").poll(
  ...
  (out) => {
    ...
    if (!ramGbSplit) return "error";

    const lineIncldes = ramGbSplit.find((line) => line.includes("Mem:"));

    if (!lineIncldes) return "error";

    return lineIncldes.split(/\s+/).splice(2, 1) + "B";
  },
);

export const disk = Variable<string>("").poll(600_000, "df -h /", (out) => {
  const lines = out.split("\n");
  if (lines.length >= 2) return lines[1].split(/\s+/)[4];
  return "error";
});
